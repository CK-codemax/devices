...
//userSchema.index({ newEmail: 1 }, { unique: true });
...
userSchema.methods.sendVerificationMessage = async (user, type) => {
  ...
  user.verificationNumber = crypto
    .createHash("sha256")
    .update(number)
    .digest("hex");
...
userSchema.methods.sendAPIKeyMessage = async (user) => {
  if (user?.emailChangedAt) return;
  const apiKey = uuidv4();
  user.apiKey = apiKey;
  console.log(apiKey);
  const message = `Your API key is ${apiKey}. Enjoy our devices API`;

  try {
    await sendEmail({
      email: user.email,
      subject: "Welcome To The Devices API",
      message,
    });
  } catch (err) {
    console.log(err);
...
userSchema.pre("save", async function (next) {
  ...
  // await this.sendAPIKeyMessage(this);
  next();
});
...
userSchema.methods.checkIfVerified = function (verifyNext) {
  return Date.now() > verifyNext;
};

userSchema.methods.verifyEmail = function (candidateVerfNum, userVerfNum) {
  return (
    crypto.createHash("sha256").update(candidateVerfNum).digest("hex") ===
    userVerfNum
  );
};
userSchema.methods.verifyApiKey = async function (candidateApiKey, userApiKey) {
  return await bcrypt.compare(userApiKey, candidateApiKey);
};
