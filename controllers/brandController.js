...
const tokenList = JSON.parse(fs.readFileSync('./data/smartphones/tokens.json', 'utf8'));
const devices = JSON.parse(fs.readFileSync('./data/smartphones/details.json', 'utf8'));
...
// });
  ...
  exports.getBrandSmartphones = catchAsync(async (req, res, next) => {
      ...
      // console.log(devices); 
        fs.writeFile('./data/smartphones/one-plus.json', JSON.stringify(devices, null, 2), { encoding: 'utf-8', flag: 'a' }, (err) => {
          if (err) {
              console.error('An error occurred:', err);
          } else {
              console.log('Content appended to the file.');
  ...
  exports.getDeviceListDetails = catchAsync(async (req, res, next) => {
      ...
      const reorderedDevices = await Promise.all(detailedDevicesPromises);
        fs.writeFile('./data/smartphones/infinix-details.json', JSON.stringify(reorderedDevices, null, 2), { encoding: 'utf-8', flag: 'a' }, (err) => {
          if (err) {
              console.error('An error occurred:', err);
          } else {
              console.log('Content appended to the file.');
    ...
    // SEND RESPONSE
      ...
      exports.fixTokens =  catchAsync(async (req, res, next) => {
        ...
        const tokenOrdered = tokenList.filter((token) =>token.logo !== null).map((token) => {
          const newToken = {
      ...
      fs.writeFile('./data/smartphones/modified-tokens.json', JSON.stringify(tokenOrdered, null, 2), { encoding: 'utf-8', flag: 'a' }, (err) => {
        if (err) {
            console.error('An error occurred:', err);
        } else {
            console.log('Content appended to the file.');
