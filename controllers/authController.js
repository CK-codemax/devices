...
exports.verifyEmail = catchAsync(async (req, res, next) => {
  ...
  if (!verificationNumber) {
    return next(
      new AppError(
        "Please provide the verification number sent to your email!",
        400
      )
    );
  }
  ...
  if (
    user.verificationNumber !== hashedToken ||
    user.verificationNumberExpires < Date.now()
  ) {
    return next(
  ...
  await user.sendAPIKeyMessage(user);
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    currentUser.changedEmailAfter(decoded.iat)
  ) {
    return next(
      new AppError(
        "User recently changed password or email! Please log in again.",
        401
      )
    );
  }
...
exports.protectAPI = catchAsync(async (req, res, next) => {
  ...
  if (!apiKey) {
    return next(
      new AppError(
        "Incorrect API key! Please use your API Key to get access.",
        401
      )
    );
  }
  ...
  // 3) Check if user still exists
  const currentUser = await User.findOne({ apiKey }).select(["+isActive"]);
  console.log(currentUser);

  if (currentUser.isActive === true && currentUser.isVerified === true) {
    next();
  } else {
    return next(new AppError("Incorrect API Key or unverified account.", 401));
  }
  // if (!currentUser.isActive)
  //   return next(new AppError("Incorrect API Key", 401));
  // if (!currentUser.isVerified)
  //   return next(new AppError("unverified account.", 401));
});
...
exports.changeEmail = catchAsync(async (req, res, next) => {
  if (!req.body.email) {
    return next(
      new AppError("Please enter the email you want to change to.", 400)
    );
  }

  if (req.body.email === req.user.email) {
    return next(
      new AppError("Please enter the new email you want to change to.", 400)
    );
  }

  const updatedUser = await User.findByIdAndUpdate(
    req.user.id,
    { $set: { newEmail: req.body.email } },
    {
      new: true,
      runValidators: true,
    }
  ...
  res.status(200).json({
    status: "success",
    message:
      "Verification number has been sent to your email, please verify your email",
    updatedUser,
  });
});
